...
vector <Edge> G[maxn];
int n, m, o, s, t, level[maxn], iter[maxn];
ll sum = 0;
...
int main() {
	...
	for(int i = 2, x; i <= n-1; ++i) x = read()*2, sum += x, add(s, i, x);
	for(int i = 2, x; i <= n-1; ++i) x = read()*2, sum += x, add(i, t, x);
	for(int x, y, a, b, c; m--; ) {
		x = read(), y = read();
		a = read(), b = read(), c = read(), sum += (a+b)*2;
		add(s, x, a), add(s, y, a);
		add(x, t, b), add(y, t, b);
		add(x, y, a+b+c*2);
		add(y, x, a+b+c*2);
