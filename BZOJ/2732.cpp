...
struct line {
	...
	line(poi _u, poi _v, int i): u(_u), v(_v) {
		sp = atan2(v.y, v.x), id = i;
	}
} a[maxn], b[maxn], q[maxn];

poi inter(line a, line b) {
	poi o = a.u - b.u;
	return a.u + a.v * ((b.v*o) / (a.v*b.v));
}

bool left(poi o, line t) { return t.v * (o-t.u) > -eps; }
...
int main() {
	...
	for(int i = 1; i <= m; ++i) {
		...
		b[++c] = line(poi(0, y2/x), poi(-1/x, 1), i);
		b[++c] = line(poi(0, y1/x), poi(1/x, -1), i);
	}
