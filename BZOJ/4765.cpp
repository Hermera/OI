...
int n, m, q, root;
int a[maxn], b[maxn], l[maxn], r[maxn], sum[maxn], blo[maxn];
...
int main() {
    ...
    for(int op, x, y, t = 1; t <= q; ++t) {
        ...
        } else x = dfn[x], lazy.push_back(ele(0, x, y - a[x])), a[x] = y;
    }
    ...
    for(int i = 1; i <= n; ++i) {
        ...
        for(int j = 0; j < wait[i].size(); ++j)
            ans[wait[i][j].t] += 1ll * query(wait[i][j].pos) * wait[i][j].val;
    }
    for(int i = 1; i <= q; ++i)
        if(ans[i]) printf("%llu\n", ans[i]);
    return 0;
