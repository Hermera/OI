...
const int maxn = 55;
const int maxl = 15;
Decimal dp[maxn][2][maxn*(maxn+1)/2];
Decimal ans[maxn];
int n = 50, m;
...
int main() {
	...
	for(int d = 0; d <= 1; ++d) 
		if(dp[i][d][j] > 0) 
			for(int k = i+1; k <= n; ++k)
				dp[k][d^1][j + (k-i)*(k-i-1)/2] += dp[i][d][j];
	
	for(int i = 1; i <= n; ++i) {
		int all = i*(i+1)/2;
		for(int k = 0; k <= i; ++k)
		for(int j = 0; j <= k*(k+1)/2; ++j) {
			int sel = j + (i-k+1)*(i-k)/2;
			if(sel == all) continue;
			if(dp[k][0][j] > 0) ans[i] -= dp[k][0][j]*all / (all-sel);
			if(dp[k][1][j] > 0) ans[i] += dp[k][1][j]*all / (all-sel);
		}
	}
	...
	while(m--) {
		cin >> n; 
		string ret = ans[n].to_string(16);
		int now = ret.size()-1, in = (ret[now] >= '5');
		for(--now; in && ~now; --now) {
			if(ret[now] == '.') continue;
			if(ret[now] == '9') ret[now] = '0';
			else ++ret[now], in = 0;
		}
		if(in) cout << "1";
